...
function parseSqlfluffJson(trimmed) {
  try {
    return JSON.parse(trimmed);
  } catch {
    throw new Error(`sqlfluff: failed to parse JSON output: ${trimmed.slice(0, 200)}`);
  }
}

function toSqlfluffFindings(fileResult) {
  const filepath = fileResult.filepath || fileResult.file;
  const violations = fileResult.violations || [];
  return violations.map(v => violationToFinding(filepath, v));
}

function violationToFinding(filepath, v) {
  const code = v.code || '';
  // Handle sqlfluff 2.x (line_no/line_pos) and 3.x (start_line_no/start_line_pos)
...
export default {
  ...
  parseOutput(stdout, stderr, exitCode) {
    ...
    const results = parseSqlfluffJson(trimmed);
    return results.flatMap(toSqlfluffFindings);
  },
